...
RGB1602_I2C = busio.I2C(board.GP9, board.GP8)
while not RGB1602_I2C.try_lock():
  pass
...
class RGB1602:
  ...
  def command(self,cmd):
    reg = 0x80
    RGB1602_I2C.writeto(LCD_ADDRESS, reg.to_bytes(1, 'big') + chr(cmd))

  def write(self,data):
    reg = 0x40
    RGB1602_I2C.writeto(LCD_ADDRESS, reg.to_bytes(1, 'big') + chr(data))

  def setReg(self,reg,data):
    RGB1602_I2C.writeto(RGB_ADDRESS, reg.to_bytes(1, 'big') + chr(data))
